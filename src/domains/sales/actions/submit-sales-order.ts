...
export async function submitSalesOrderAction(input: unknown) {
  ...
  try {
    ...
    if (!allowedRoles.has(context.role)) {
      throw new BusinessError("You do not have permission to submit sales orders.", "FORBIDDEN");
    }

    await new SalesOrderService().submit(context, parsed.data.id);
