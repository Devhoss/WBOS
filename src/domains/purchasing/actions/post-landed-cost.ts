...
const postLandedCostSchema = z.object({
  id: z.string().trim().min(1, "Landed cost is required."),
});

export async function postLandedCost(input: unknown) {
  const parsed = postLandedCostSchema.safeParse(input);
  ...
  try {
    ...
    const landedCost = await new LandedCostService().post(context, parsed.data.id);
