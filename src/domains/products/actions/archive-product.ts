...
import { archiveProductSchema } from "../validation/product-schema";

export async function archiveProduct(input: unknown) {
  ...
  try {
    ...
    await new ProductService().archive(context, parsed.data.id);
