...
export function CustomerForm() {
  ...
  function handleSubmit(formData: FormData) {
    ...
    startTransition(async () => {
      ...
      if (!result.ok) {
        setMessage(result.message ?? "Unable to create customer.");
        return;
      }
    ...
    <form action={handleSubmit} className="rounded-lg border p-5">
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        ...
        <label className="space-y-2 text-sm">
          ...
          <input className="h-10 w-full rounded-md border bg-background px-3 text-sm outline-none focus:border-primary" min="0" name="creditLimit" step="0.001" type="number" />
        </label>
        <label className="space-y-2 text-sm">
