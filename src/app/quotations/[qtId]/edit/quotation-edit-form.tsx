...
export function QuotationEditForm({
  ...
  const [lines, setLines] = useState<QtLine[]>(
  ...
  const [taxAmount, setTaxAmount] = useState(String(quotation.taxAmount));
  ...
  const [discountType, setDiscountType] = useState<"" | "PERCENTAGE" | "FIXED">((quotation.discountType as "" | "PERCENTAGE" | "FIXED") ?? "");
  const [discountRate, setDiscountRate] = useState(quotation.discountRate ? String(quotation.discountRate) : "");
  ...
  async function submit() {
    ...
    try {
      ...
      if (!result.ok) {
        setMessage(result.message ?? "Unable to update quotation.");
        setIsPending(false);
        return;
      }
  ...
  return (
    <section className="rounded-lg border p-5">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h2 className="text-base font-semibold">Edit Quotation</h2>
        </div>
        <button
        ...
        >
          <Plus className="size-4" />{isPending ? "Saving..." : "Save"}
        </button>
      </div>
